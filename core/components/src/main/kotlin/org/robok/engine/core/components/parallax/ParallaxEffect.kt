...
class ParallaxEffect(private val context: Context) : SensorEventListener {
  ...
  override fun onSensorChanged(event: SensorEvent) {
    ...
    var pitch = atan2(x, sqrt(y * y + z * z)) / Math.PI.toFloat() * 2f
    var roll = atan2(y, sqrt(x * x + z * z)) / Math.PI.toFloat() * 2f
    ...
    if (roll > 1f) roll = 2f - roll else if (roll < -1f) roll = -2f - roll
    ...
    angle.value =
      (atan2(vx * -1f - vy * 0f, vx * 0f + vy * -1f) / (PI / 180f)).toFloat().let {
