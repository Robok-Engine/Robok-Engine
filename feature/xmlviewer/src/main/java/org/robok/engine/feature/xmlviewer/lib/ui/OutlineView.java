  ...
  public boolean onTouchEvent(MotionEvent event) {
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        for (Pair<View, Rect> pair : pairArrayList) {
          if (isInRect(pair.second, event.getRawX(), event.getRawY())) {
            ...
            if (outlineClickListener != null) outlineClickListener.onDown(selectView, displayType);
            invalidate();
            return true;
          }
        }
      ...
      case MotionEvent.ACTION_CANCEL:
        if (isSelect) {
          if (outlineClickListener != null) outlineClickListener.onCancel(selectView, displayType);
          if (!isHoldOutline) {
            removeSelect();
          }
        }
      ...
      case MotionEvent.ACTION_UP:
        ...
        // 必须是在rect内
        if (isSelect) {
          if (isInRect(selectedRect, event.getRawX(), event.getRawY())
              && outlineClickListener != null) {
