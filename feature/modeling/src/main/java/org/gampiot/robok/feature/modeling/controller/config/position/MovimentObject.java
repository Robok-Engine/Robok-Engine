package org.gampiot.robok.feature.modeling.controller.config.position;
...
public class MovimentObject {
            ...
            if (selectedObject != null) {
                ...
                if (draggedArrow != null) {
                   // CameraInputController2.teste = false;
                    isDragging = true;
                    dragStartPosition.set(selectedObject.getModelInstance().transform.getTranslation(new Vector3()));
    ...
    private Arrow getArrowAtScreenCoordinates(Vector2 touchPos) {
        for (Arrow arrow : movementArrows) {
            ...
            Vector3 screenEnd = camera.project(arrow.position.cpy().add(arrow.direction.cpy().scl(arrow.length)));

            if (touchPos.dst(screenStart.x, screenStart.y) < 100 || touchPos.dst(screenEnd.x, screenEnd.y) < 100) {
                return arrow;
            }
        }
        return null;
    ...
    private void updateMovementArrows() {
        ...
        movementArrows.add(new Arrow(center.cpy().add(dimensions.x / 2 + arrowLength / 2, 0, 0), Vector3.X, Color.RED, arrowLength));
        movementArrows.add(new Arrow(center.cpy().add(0, dimensions.y / 2 + arrowLength / 2, 0), Vector3.Y, Color.GREEN, arrowLength));
        movementArrows.add(new Arrow(center.cpy().add(0, 0, dimensions.z / 2 + arrowLength / 2), Vector3.Z, Color.BLUE, arrowLength));
    }
